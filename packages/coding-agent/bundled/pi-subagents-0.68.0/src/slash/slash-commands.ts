...
export function registerSlashCommands(
	...
	pi.registerCommand("subagents-status", {
		description: "Inspect a subagent run: /subagents-status [run-id]",
		handler: async (args, ctx) => {
			const id = args.trim();
			await runSlashSubagent(pi, ctx, { action: "status", ...(id ? { id } : {}) });
		},
	});

	pi.registerCommand("subagents-interrupt", {
		description: "Pause a subagent run: /subagents-interrupt [run-id]",
		handler: async (args, ctx) => {
			const id = args.trim();
			await runSlashSubagent(pi, ctx, { action: "interrupt", ...(id ? { id } : {}) });
		},
	});

	pi.registerCommand("subagents-stop", {
		description: "Stop a subagent run: /subagents-stop [run-id]",
		handler: async (args, ctx) => {
			const id = args.trim();
			await runSlashSubagent(pi, ctx, { action: "stop", ...(id ? { id } : {}) });
		},
	});

	pi.registerCommand("subagents-resume", {
		description: "Resume a subagent run: /subagents-resume <run-id> -- <message>",
		handler: async (args, ctx) => {
			const delimiter = args.indexOf(" -- ");
			if (delimiter < 1) {
				ctx.ui.notify("Usage: /subagents-resume <run-id> -- <message>", "error");
				return;
			}
			const id = args.slice(0, delimiter).trim();
			const message = args.slice(delimiter + 4).trim();
			if (!id || !message) {
				ctx.ui.notify("Usage: /subagents-resume <run-id> -- <message>", "error");
				return;
			}
			await runSlashSubagent(pi, ctx, { action: "resume", id, message });
		},
	});
