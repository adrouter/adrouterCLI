...
export function resolveTopLevelParallelMaxTasks(value: unknown): number {
	return Math.min(normalizeTopLevelParallelValue(value) ?? MAX_PARALLEL, MAX_PARALLEL);
}
...
export function resolveTopLevelParallelConcurrency(
	...
	return Math.min(normalizeTopLevelParallelValue(override)
...
export function resolveCurrentMaxSubagentDepth(configMaxDepth?: number): number {
	const requested = normalizeMaxSubagentDepth(process.env.PI_SUBAGENT_MAX_DEPTH)
		?? normalizeMaxSubagentDepth(configMaxDepth)
		?? DEFAULT_SUBAGENT_MAX_DEPTH;
	return Math.min(DEFAULT_SUBAGENT_MAX_DEPTH, requested);
}
