...
function findSkillFile(dir: string): string | undefined {
	for (const candidate of [path.join(dir, "SKILL.md"), path.join(dir, "docs", "SKILL.md")]) {
...
function collectFilesystemSkills(cwd: string, agentDir: string, skillPaths: SkillSearchPath[]): CachedSkillEntry[] {
	...
	for (const skillPath of skillPaths) {
		...
		const rootSkillFile = findSkillFile(skillPath.path);
		if (rootSkillFile) {
		...
		for (const child of childEntries) {
			...
			if (child.isDirectory() || child.isSymbolicLink()) {
				const nestedSkillPath = findSkillFile(childPath);
				if (nestedSkillPath) {
