...
import { buildAdRouterChildProcessEnv, buildPiArgs, cleanupTempDir } from "../shared/pi-args.ts";
...
import { consumeAsyncStop } from "./stop-control.ts";
import { createInterruptRegistry } from "./interrupt-registry.ts";
...
function runPiStreaming(
	...
	return new Promise((resolve) => {
		...
		const spawnEnv = buildAdRouterChildProcessEnv({
			...(env ?? {}),
...
async function runSubagent(config: SubagentRunConfig): Promise<void> {
	...
	if (
		config.share === true ||
		config.worktreeSetupHook !== undefined ||
		steps.some((step) => isParallelGroup(step) && step.worktree === true)
	) {
		throw new Error("Gist sharing and managed worktrees are outside the reviewed AdRouter subagent subset.");
	}
	let previousOutput = "";
	...
	const activeChildInterrupts = createInterruptRegistry();
	let interrupted = false;
	let stopped = false;
	...
	const interruptRunner = () => {
		...
		activeChildInterrupts.interruptAll();
	};
	const stopRunner = () => {
		if (stopped || statusPayload.state !== "running") return;
		stopped = true;
		interrupted = true;
		const now = Date.now();
		statusPayload.state = "failed";
		statusPayload.error = "Stopped by user.";
		currentActivityState = undefined;
		statusPayload.activityState = undefined;
		statusPayload.lastUpdate = now;
		for (const step of statusPayload.steps) {
	...
	if (consumeAsyncStop(asyncDir)) stopRunner();
	...
	while (true) {
		...
		if (isDynamicRunnerGroup(step)) {
			...
			const concurrency = Math.min(step.concurrency ?? MAX_PARALLEL_CONCURRENCY, MAX_PARALLEL_CONCURRENCY);
			...
			const parallelResults = await mapConcurrent(dynamicSteps, concurrency, async (task, taskIdx) => {
				...
				const singleResult = await runSingleStep(task, {
					...
					registerInterrupt: activeChildInterrupts.createRegistrar(),
		...
		if (isParallelGroup(step)) {
			const group = step;
			const concurrency = Math.min(group.concurrency ?? MAX_PARALLEL_CONCURRENCY, MAX_PARALLEL_CONCURRENCY);
	...
	if (stopRequestTimer) {
		...
		stopRequestTimer = undefined;
	}
	process.off(ASYNC_INTERRUPT_SIGNAL, interruptRunner);
	const effectiveSessionFile = sessionFile ?? latestSessionFile;
	const runEndedAt = Date.now();
	statusPayload.state = stopped ? "failed" : interrupted ? "paused" : results.every((r) => r.success) ? "complete" : "failed";
	statusPayload.activityState = undefined;
	...
	if (stopped) {
		statusPayload.error = "Stopped by user.";
	} else if (statusPayload.state === "failed" && !statusPayload.error) {
	...
	try {
		writeAtomicJson(resultPath, {
			...
			agent: agentName,
			mode: resultMode,
			success: !stopped && !interrupted && results.every((r) => r.success),
			state: stopped ? "failed" : interrupted ? "paused" : results.every((r) => r.success) ? "complete" : "failed",
			summary: stopped
				? "Stopped by user."
				: interrupted
			...
			exitCode: stopped ? 1 : interrupted || results.every((r) => r.success) ? 0 : 1,
