...
import { requestAsyncStop } from "../background/stop-control.ts";
...
const ADROUTER_MAX_PARALLEL_CHILDREN = 3;
const ADROUTER_READ_ONLY_CHILD_TOOLS = new Set([
	"read",
	"grep",
	"find",
	"ls",
...
function stopAsyncRun(
	...
	const direct = runId ? state.asyncJobs.get(runId) : undefined;
	const target = location?.asyncDir
		? { asyncId: location.resolvedId ?? runId ?? path.basename(location.asyncDir), asyncDir: location.asyncDir }
		: direct
			? { asyncId: direct.asyncId, asyncDir: direct.asyncDir }
			: getAsyncInterruptTarget(state, runId);
	if (!target) return null;
	const status = readStatus(target.asyncDir);
	...
	try {
		requestAsyncStop(target.asyncDir);
		const tracked = state.asyncJobs.get(target.asyncId);
		if (tracked) {
			tracked.activityState = undefined;
			tracked.updatedAt = Date.now();
		...
		return {
			content: [
				{
					type: "text",
					text: `Failed to stop async run ${target.asyncId}: ${error instanceof Error ? error.message : String(error)}`,
				},
			],
			isError: true,
			details: { mode: "management", results: [] },
...
function isMutationCapableAgent(agent: AgentConfig): boolean {
	if (!agent.tools || agent.tools.length === 0) return true;
	if (agent.extensions?.length || agent.subagentOnlyExtensions?.length || agent.mcpDirectTools?.length) return true;
	return agent.tools.some((tool) => !ADROUTER_READ_ONLY_CHILD_TOOLS.has(tool));
}

function hasObjectAcceptance(params: SubagentParamsLike): boolean {
	const isObject = (value: unknown) => typeof value === "object" && value !== null;
	if (isObject(params.acceptance)) return true;
	if (params.tasks?.some((task) => isObject(task.acceptance))) return true;
	return params.chain?.some((step) => {
		if (isObject(step.acceptance)) return true;
		return isParallelStep(step) && step.parallel.some((task) => isObject(task.acceptance));
	}) ?? false;
}

export function validateAdRouterParallelPolicy(
	params: SubagentParamsLike,
	agents: AgentConfig[],
): AgentToolResult<Details> | null {
	const byName = new Map(agents.map((agent) => [agent.name, agent]));
	const groups: Array<{ label: string; names: string[] }> = [];
	if (params.tasks) groups.push({ label: "Top-level parallel run", names: params.tasks.map((task) => task.agent) });
	for (const [index, step] of (params.chain ?? []).entries()) {
		if (isDynamicParallelStep(step)) {
			return buildRequestedModeError(
				params,
				`Chain step ${index + 1} uses dynamic fanout, which is outside the reviewed AdRouter subagent subset.`,
			);
		}
		if (isParallelStep(step)) {
			groups.push({ label: `Chain parallel step ${index + 1}`, names: step.parallel.map((task) => task.agent) });
		}
	}

	for (const group of groups) {
		if (group.names.length > ADROUTER_MAX_PARALLEL_CHILDREN) {
			return buildRequestedModeError(
				params,
				`${group.label} exceeds the hard limit of ${ADROUTER_MAX_PARALLEL_CHILDREN} children.`,
			);
		}
		const mutationCapable = group.names.filter((name) => {
			const agent = byName.get(name);
			return !agent || isMutationCapableAgent(agent);
		});
		if (mutationCapable.length > 1) {
			return buildRequestedModeError(
				params,
				`${group.label} may contain at most one mutation-capable child; ` +
					`use read/grep/find/ls-only profiles for the remaining children. Mutation-capable: ${mutationCapable.join(", ")}.`,
			);
		}
	}
	return null;
...
export function createSubagentExecutor(deps: ExecutorDeps): {
	...
	const execute = async (
		...
		if (
			...
			params.chain?.some((step) => "worktree" in step && step.worktree === true) ||
			params.control?.notifyChannels?.includes("intercom") ||
			hasObjectAcceptance(params)
		) {
			return {
				content: [
					{
						type: "text",
						text: "Gist sharing, managed worktrees, external intercom, and object-form executable acceptance policies are outside the reviewed AdRouter subagent subset.",
					},
				],
				isError: true,
				details: { mode: "management", results: [] },
		...
		if (params.action) {
			if (!(SUBAGENT_ACTIONS as readonly string[]).includes(params.action)) {
				return {
					content: [{ type: "text", text: `Action '${params.action}' is outside the reviewed AdRouter subagent subset. Valid: ${SUBAGENT_ACTIONS.join(", ")}` }],
					isError: true,
			...
			if (params.action === "children") {
				return inspectSubagentStatus({}, { state: deps.state, nested: nestedResolutionScopeForExecutor(deps) });
			}
			if (params.action === "models") {
				const models = ctx.modelRegistry.getAvailable()
					...
					.sort();
				return {
					content: [{ type: "text", text: models.length ? `Available models:\n${models.map((model) => `- ${model}`).join("\n")}` : "No models are currently available." }],
					details: { mode: "management", results: [] },
				};
			}
			...
			if (params.action === "stop") {
				...
				if (resolved?.kind === "nested") {
					return {
						content: [{ type: "text", text: "Nested delegation is disabled in the AdRouter subagent subset." }],
						isError: true,
						details: { mode: "management", results: [] },
					};
				}
				...
				if (foreground?.interrupt) {
					const stopped = foreground.interrupt();
					return {
						content: [
							{
								type: "text",
								text: stopped
									? `Stop requested for foreground run ${foreground.runId}; its active child was interrupted.`
									: `Foreground run ${foreground.runId} has no active child step to stop.`,
							},
						],
						isError: stopped ? undefined : true,
						details: { mode: "management", results: [] },
					};
				}
				...
				return {
					content: [{ type: "text", text: "No stoppable run found in this session." }],
					isError: true,
					details: { mode: "management", results: [] },
				};
			}
		...
		const agents = intercomBridge.active
			? discoveredAgents.map((agent) => applyIntercomBridgeToAgent(agent, intercomBridge))
			: discoveredAgents;
		const adRouterPolicyError = validateAdRouterParallelPolicy(effectiveParams, agents);
		if (adRouterPolicyError) return adRouterPolicyError;
		const runId = randomUUID().slice(0, 8);
