...
export const PI_CODING_AGENT_PACKAGE = "@adrouter/cli";
...
export function getPiSpawnCommand(args: string[], deps: PiSpawnDeps = {}): PiSpawnCommand {
	...
	// Bundled subagents must re-enter the current AdRouterCLI installation so
	// they retain its router configuration and do not require a separate Pi
	// binary on PATH. The Windows branch above still prefers the active script
	// when it can resolve one.
	return { command: "adrouter", args };
}
